...
class ArticleManager(models.Manager):
    def distinct_date(self):  # 该管理器定义了一个distinct_date方法，目的是找出所有的不同日期
        ...
        date_list = self.values('time').order_by('-time')  # 根据文章字段time找出所有文章的发布时间
        for date in date_list:  # 对所有日期进行遍历，当然这里会有许多日期是重复的，目的就是找出多少种日期
            date = date['time'].strftime('%Y{}%m{} {}'.format('年','月','存档')) # 取出一个日期改格式为 ‘xxx年/xxx月 存档’
...
class Post(models.Model):
    ...
    type = models.CharField('类型',max_length=8,default=ORIGINAL,
                            choices=((ORIGINAL, '原创'),(REPRINT, '转载'),(TALK, '杂谈')))

...
class UPfile(models.Model):
    ...
    Prohibited_info = models.CharField(
        '删除信息', blank=False, default='违规', max_length=256)
    user = models.ForeignKey(
        MyUser,
        on_delete=models.CASCADE,
        verbose_name='用户')
...
class Valid(models.Model):
    id = models.AutoField('序号', primary_key=True)
    value = models.CharField('验证码', default='', max_length=127)
    is_valid = models.NullBooleanField('是否验证', default=False)
    user = models.ForeignKey(
        MyUser,
        on_delete=models.CASCADE,
        verbose_name='用户')

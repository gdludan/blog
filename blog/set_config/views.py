...
def edit_profileView(request):
    ...
    if not profile:
        profile = Profile(user=user, self_reprot='这个人很懒，什么也没有写！')  # 防止出现错误
...
def edit_gravatarView(request):
    ...
    if request.method == 'POST':
        ...
        if platform.system() != 'Windows':
            os.system('rm -rf ' + settings.BASE_DIR + user.avatar)
        else:
            os.remove(settings.BASE_DIR + user.avatar)
